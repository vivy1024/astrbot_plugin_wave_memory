...
function FewShotDetail({ item }: { item: ApprovedFewShot }) {
  return <div className="flex flex-col gap-4">
    <div className="flex flex-wrap gap-2"><Badge>已通过</Badge><Badge variant="secondary">内容健康</Badge><Badge variant="outline">Bot {item.bot_id}</Badge></div>
    ...
    <div><h3 className="mb-2 text-sm font-medium text-muted-foreground">风格特征</h3><div className="flex flex-wrap gap-1">{item.traits?.length ? item.traits.map((trait) => <Badge key={trait} variant="outline">{trait}</Badge>) : <span className="text-muted-foreground">未记录风格特征</span>}</div></div>
  </div>
}
...
export function FewShotPage() {
  ...
  return <div className="flex flex-col gap-4" data-page="few-shot">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <header className="max-w-2xl"><h1 className="text-xl font-bold tracking-tight">风格样例</h1><p className="text-xs text-muted-foreground">只展示当前 Bot 和群里已通过审核、且内容健康的正式范例。</p></header>
      <div className="flex flex-wrap gap-2 text-xs"><SummaryTile label="筛选总数" value={total} /><SummaryTile label="本页平均分" value={averageScore} tone="text-emerald-600" /><SummaryTile label="本页风格特征" value={traitCount} /></div>
    </div>
        ...
        <QueryState status={status} error={error} title="风格样例读取失败" description={!botId || !sessionId ? '请先选择 Bot 和群聊。' : '当前群暂无已通过审核的风格样例。'} onRetry={() => setReload((value) => value + 1)}>
          <DeclarativeDataTable
            ...
            columns={[
              ...
              { key: 'traits', header: '风格特征', render: (row) => <div className="flex max-w-64 gap-1 overflow-hidden">{row.traits?.length ? row.traits.slice(0, 3).map((trait) => <Badge key={trait} variant="outline" className="max-w-24 truncate">{trait}</Badge>) : <span className="text-xs text-muted-foreground">未记录</span>}</div> },
              { key: 'status', header: '正式状态', render: () => <Badge>已通过 / 内容健康</Badge> },
              { key: 'actions', header: null, render: (row) => <ResponsiveDetail title="风格样例" description={`Bot ${row.bot_id} 的只读正式范例`} trigger={<Button type="button" variant="ghost" size="icon-sm" aria-label="查看风格样例详情"><EyeIcon aria-hidden="true" /></Button>}><FewShotDetail item={row} /></ResponsiveDetail> },
            ]}
            renderCardActions={(row) => <ResponsiveDetail title="风格样例" description={`Bot ${row.bot_id} 的只读正式范例`} trigger={<Button type="button" variant="outline" size="sm">查看详情</Button>}><FewShotDetail item={row} /></ResponsiveDetail>}
          />
        </QueryState>
      {data?.page ? <div className="border-t px-4 py-3"><PaginationControls page={data.page} onOffsetChange={pagination.setOffset} onLimitChange={pagination.setLimit} disabled={loading} label="风格样例分页" /></div> : null}
    </CardContent></Card>
    ...
    <details className="rounded-lg border border-dashed text-sm">
      ...
      <p className="border-t px-4 py-3 text-xs leading-relaxed text-muted-foreground">本页不展示候选、拒绝项或审核写操作。除已通过审核外，内容还须通过身份污染与不安全风格检查；总数、分页和搜索始终限定在当前 Bot 和群。</p>
    </details>
  </div>
}
