...
export function TagGraphControls({
  botId,
  sessionId,
  layers,
  includePulse,
  ...
  return (
    <Card className="border-sky-950/70 bg-sky-950/[.08] shadow-sm">
      <CardContent className="flex flex-col gap-3 p-4">
        {/* 第一行：群与 Bot 选择 */}
        <div className="flex flex-wrap items-end gap-3">
          <Badge variant="outline" className="mb-1">当前群</Badge>
          <ScopeSelect
            className="min-w-48 flex-1 xl:max-w-64"
            value={botId || undefined}
            loadOptions={loadBots}
            label="Bot"
            placeholder="选择 Bot"
            required
            onValueChange={(value) => onScopeChange({ botId: value, sessionId: '' })}
          />
          <ScopeSelect
            className="min-w-56 flex-[1.3] xl:max-w-80"
            value={sessionId || undefined}
            loadOptions={loadSessions}
            label="群 / 会话"
            placeholder="选择该 Bot 的群"
            disabled={!botId}
            required
        ...
        {/* 第二行：图层与脉冲 */}
        <div className="flex flex-wrap items-center gap-2 border-t pt-3" aria-label="标签图层与控制">
          ...
          <label className="flex items-center gap-2 text-xs text-muted-foreground ml-2">
            <WavesIcon className="size-4 text-sky-400" aria-hidden="true" />
            脉冲能量
            <Switch
