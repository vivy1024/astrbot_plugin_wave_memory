...
- **心智强制自省入口**：`POST /api/soul/state/refresh` 按当前群重算只读投影，不写库、不调 LLM、不改 revision；心智页提供「刷新数据 / 强制自省」。
- **表格双写根治**：新增 `DeclarativeDataTable`，一列定义同时驱动桌面表和手机卡片；信念、黑话、记忆、人物、事实、风格样例、注入、索引、维护、标签等页迁出手工双写。
- **筛选与批量操作抽取**：抽出 `ScopeFilterBar`、`BatchActionBar`，减少 Bot/群筛选与批量审核的重复 JSX。
- **可见文案去内部词**：时间线、事实、风格样例、标签图、记忆、信念、黑话、注入观测台、人物、导入等页把 Canonical Scope / ObjectRef / Affinity / Trace / Holyman 等后端词改成中文，保留 Bot、群、记忆、标签。

### 人物关系与证据

- **人物历史关系审计**：只读 `/api/people/relationships/historical-audit`，展示本群历史关系事件，不参与好感计算。
- **关系校准证据**：校准必须选择本群真实记忆；证据解析与群选项按生产 Scope 对齐。
- **好感筛选与校准面板**：人物页支持按好感范围/是否已记录关系筛选，并展示自动学习、人工调整与最终生效值。
